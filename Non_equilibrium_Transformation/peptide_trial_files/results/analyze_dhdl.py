...
def parse_dgdl_files(list[0], lambda0=0, invert_values=False):
    ...
    #imax = _longest_dgdl_file(list)

    _check_dgdl(list[0], lambda0)
    first_w, ndata = integrate_dgdl(list[0], lambda0=lambda0,
                                    invert_values=invert_values)
    w_list = [first_w]
    for idx, f in enumerate(list[1:]):
...
def integrate_dgdl(fn, ndata=-1, lambda0=0, invert_values=False):
    ...
    if invert_values is True:
        integr = integrate.simps(y,x) * (-1)
        return integr, ndata
    else:
        integr = integrate.simps(y,x)
        return integr, ndata
